        ...
        static void Main(string[] args)
        {
            ...
            if (!string.IsNullOrEmpty(text))
            {
                ...
                if (indvLines.Length > 0)
                {
                    ...
                    List<KeyValuePair<Board,int>> winners = new List<KeyValuePair<Board, int>>();

                    for (int i  = 0; i < nums.Length; i++)
                    {
                        for (int board = boards.Count - 1; board >= 0; board--)
                        {
                            if (boards[board].InputValue(nums[i]))
                            {
                                winners.Add(new KeyValuePair<Board, int>(boards[board], nums[i]));
                    ...
                    // Step 4: final result
                    Console.WriteLine("Final result is: " + winners[winners.Count -1].Key.GetFinalValue() * winners[winners.Count - 1].Value);
                }
            }
            else
            {
